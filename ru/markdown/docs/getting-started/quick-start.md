---
title: Начало
---

## Установка

Давайте начнём с установки Scratch Addons.

| Магазин | Установить | Поддерживаемые браузеры |
| - | - | - |
| Веб-магазин Chrome | [![Установить для Веб-магазина Chrome](https://img.shields.io/chrome-web-store/v/fbeffbjdlemaoicjdapfpikkikjoneco?style=flat-square&logo=google-chrome&logoColor=white&label=install&color=4285F4)](https://chrome.google.com/webstore/detail/fbeffbjdlemaoicjdapfpikkikjoneco) | Google Chrome и другие браузеры, основанные на Chromium
| Дополнения для Firefox | [![Скачать для Дополнений для Firefox](https://img.shields.io/amo/v/scratch-messaging-extension?style=flat-square&logo=firefox-browser&logoColor=white&label=install&color=FF7139)](https://addons.mozilla.org/firefox/addon/scratch-messaging-extension/) | Mozilla Firefox
| Дополнения Microsoft Edge | [![Скачать для Дополнений Microsoft Edge](https://img.shields.io/badge/dynamic/json?style=flat-square&logo=data:image/svg+xml;base64,&label=install&color=0078D7&prefix=v&query=%24.version&url=https%3A%2F%2Fmicrosoftedge.microsoft.com%2Faddons%2Fgetproductdetailsbycrxid%2Filiepgjnemckemgnledoipfiilhajdjj)](https://microsoftedge.microsoft.com/addons/detail/iliepgjnemckemgnledoipfiilhajdjj) | Microsoft Edge

Больше информации можно найти на [Установке](../installing).


## Давайте начнём

Чтобы приступить, Вам надо получить доступ к странице настроек расширения, чтобы посмотреть его функции.

1. Найдите Scratch Addons на строке инструментов и нажмите на него.

   Обычно, она на верхнем правом углу Вашего браузера. Для пользователей Google Chrome, возможно, Вам понадобиться нажать на иконку пазла, чтобы найти иконку расширения.

   ![](/assets/img/getting-started/step-1.png)

2. Щёлкните на иконку шестерёнки для перехода на страницу Настроек.

   ![](/assets/img/getting-started/step-2.png)

3. Можете пробегать через список дополнений, включать, выключать что угодно.

   Вы также можете конфигурировать дополнение в его настройках, которые откроются при активировании или раскрытии.

   ![](/assets/img/getting-started/step-3.png)